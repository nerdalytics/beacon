...
describe("Performance", { concurrency: true }, (): void => {
	...
	it("should handle many dependencies", async (): Promise<void> => {
		...
		const sources = Array.from(
			{ length: COUNT },
			(_: unknown, i: number): Signal<number> => state(i),
		);

		// Create a derived signal that depends on all sources
		const sum = derived((): number => {
			return sources.reduce(
				(acc: number, source: Signal<number>): number => acc + source(),
				0,
			);
		});
		...
		if (initial !== expected) {
			throw new Error(
				`Initial sum incorrect: expected ${expected}, got ${initial}`,
			);
		}
		...
		for (let iter = 0; iter < ITERATIONS; iter++) {
			...
			if (value !== expectedSum) {
				throw new Error(
					`Sum incorrect: expected ${expectedSum}, got ${value}`,
				);
			}
		}
		const end = performance.now();
	...
	it("should compare batch vs. unbatched updates", (): void => {
		...
		// With batch
		const batchedSignals = Array.from(
			{ length: SIGNAL_COUNT },
			(): Signal<number> => state(0),
		);
		...
		// Without batch
		const unbatchedSignals = Array.from(
			{ length: SIGNAL_COUNT },
			(): Signal<number> => state(0),
		);
		...
		const unbatchedOps =
			((UPDATES * SIGNAL_COUNT) / unbatchedElapsed) * 1000;
		...
		console.log(
			`Performance ratio: ${(unbatchedElapsed / batchedElapsed).toFixed(2)}x faster with batching`,
		);
		console.log(
			`Effect runs ratio: ${(unbatchedEffectCount / batchedEffectCount).toFixed(2)}x fewer with batching`,
		);
	});
});
