...
describe("Cyclic Dependencies", { concurrency: true }, (): void => {
	it("should handle direct cyclic dependencies between two state signals", (): void => {
		...
		assert.ok(
			executionLog.length < 20,
			`Too many executions (${executionLog.length}) before stabilizing`,
		);
	...
	it("should demonstrate derived signals with potential cycles", (): void => {
		...
		console.log(
			updateCount < 100
				? "✓ System eventually stabilized"
				: "! System didn't stabilize within 100 updates",
		);
	});
	...
	it("should demonstrate three interlocked states in a cycle (A → B → C → A)", (): void => {
		...
		// For debugging and analysis
		console.log(
			`Interlocked three-state cycle - Total updates: ${updateCount}`,
		);
		console.log(
			`Final values - A: ${signalA()}, B: ${signalB()}, C: ${signalC()}`,
		);
		console.log(`First ${valueHistory.length} states:`, valueHistory);

		// Check values are still finite (not NaN or Infinity)
		assert.ok(
			Number.isFinite(signalA()),
			`A should have a finite value, got ${signalA()}`,
		);
		assert.ok(
			Number.isFinite(signalB()),
			`B should have a finite value, got ${signalB()}`,
		);
		assert.ok(
			Number.isFinite(signalC()),
			`C should have a finite value, got ${signalC()}`,
		);
	...
	it("should demonstrate tree structures with a parent affecting multiple children (A → B, A → C)", (): void => {
		...
		const childB = derived(() => parent() * 2);
		const childC = derived(() => parent() / 2);
		...
		assert.strictEqual(
			bUpdateCount,
			3,
			"Child B should update once per parent change",
		);
		assert.strictEqual(
			cUpdateCount,
			3,
			"Child C should update once per parent change",
		);
		assert.strictEqual(
			totalUpdateCount,
			6,
			"Total updates should be sum of child updates",
		);
		...
		// For analysis
		console.log(
			`Tree structure test - B updates: ${bUpdateCount}, C updates: ${cUpdateCount}`,
		);
		console.log(
			`Final values - A: ${parent()}, B: ${childB()}, C: ${childC()}`,
		);
	...
	it("should handle linked leaves within a tree (A → B, A → C, B → D, D → A)", (): void => {
		...
		console.log(
			`Initial values - A: ${initialA}, B: ${initialB}, C: ${initialC}, D: ${initialD}`,
		);
		...
		console.log(
			`Value history (first ${valueHistory.length}):`,
			valueHistory,
		);
		...
		assert.ok(
			Number.isFinite(a()),
			`A should have a finite value, got ${a()}`,
		);
		assert.ok(
			Number.isFinite(b()),
			`B should have a finite value, got ${b()}`,
		);
		assert.ok(
			Number.isFinite(c()),
			`C should have a finite value, got ${c()}`,
		);
		assert.ok(
			Number.isFinite(d()),
			`D should have a finite value, got ${d()}`,
		);
	...
	it("should analyze convergence behavior in cyclic dependencies", (): void => {
		...
		results.forEach((result) => {
			...
			console.log(
				`Converged: ${result.converged ? "Yes" : "No"} (${result.totalUpdates} updates)`,
			);
