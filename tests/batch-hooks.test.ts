...
import { batch, state } from '../src/index.ts'
import type { BatchHooks } from '../src/hooks/index.ts'

describe('Batch Hooks', { concurrency: true, timeout: 1000 }, () => {
	...
	it('fires onBatchStart with correct depth', () => {
		const depths: number[] = []
		batch(() => {}, {
			onBatchStart: (depth) => { depths.push(depth) },
		})
	...
	it('fires onBatchEnd with correct depth', () => {
		const depths: number[] = []
		batch(() => {}, {
			onBatchEnd: (depth) => { depths.push(depth) },
		})
		assert.deepStrictEqual(depths, [1])
	})
	...
	it('fires onBatchError with error and depth', () => {
		...
		assert.throws(() => {
			batch(() => { throw new Error('batch fail') }, {
				onBatchError: (err, depth) => { errors.push([err, depth]) },
			})
		}, { message: 'batch fail' })
	...
	it('reports correct depths for nested batches', () => {
		...
		const ends: number[] = []
		const outerHooks = {
			onBatchEnd: (d: number): void => { ends.push(d) },
			onBatchStart: (d: number): void => { starts.push(d) },
		}
		const innerHooks = {
			onBatchEnd: (d: number): void => { ends.push(d) },
			onBatchStart: (d: number): void => { starts.push(d) },
		...
		assert.deepStrictEqual(ends, [2, 1])
	})

	it('isolates hook errors from batch execution', () => {
		const result = batch(() => 99, {
			onBatchStart: () => { throw new Error('hook error') },
		})
