...
import type { EffectHooks } from '../src/hooks/index.ts'

describe('Effect Hooks', { concurrency: true, timeout: 1000 }, () => {
	...
	it('works with name and no hooks (backward compat)', () => {
		let ran = false
		const dispose = effect(() => { ran = true }, 'test')
		assert.strictEqual(ran, true)
		dispose()
	})
	...
	it('fires onRun with effectName', () => {
		...
		const dispose = effect(() => {}, 'myEffect', {
			onRun: (name) => { names.push(name) },
		})
		assert.deepStrictEqual(names, ['myEffect'])
		dispose()
	})
	...
	it('fires onDispose with effectName', () => {
		const names: (string | undefined)[] = []
		const dispose = effect(() => {}, 'myEffect', {
			onDispose: (name) => { names.push(name) },
		})
		dispose()
		assert.deepStrictEqual(names, ['myEffect'])
	})
	...
	it('fires onError when effect throws and re-throws', () => {
		const errors: Error[] = []
		assert.throws(() => {
			effect(() => { throw new Error('boom') }, 'failing', {
	...
	it('fires onDependencyAdd for new dependencies', () => {
		const deps: [object, PropertyKey, string | undefined][] = []
		const $s = state({ count: 0 })
		const dispose = effect(() => {
			const _v = $s.count
		}, 'tracker', {
			onDependencyAdd: (target, prop, name) => { deps.push([target, prop, name]) },
		})
		assert.ok(deps.length > 0)
		assert.strictEqual(deps.some((d) => d[1] === 'count'), true)
	...
	it('fires onSchedule when effect is queued for re-run', () => {
		...
		const dispose = effect(() => {
			...
			onSchedule: (name) => { schedules.push(name) },
		})
		$s.count = 1
	...
	it('isolates hook errors from effect execution', () => {
		let ran = false
		const dispose = effect(() => { ran = true }, 'safe', {
			onRun: () => { throw new Error('hook error') },
		})
		assert.strictEqual(ran, true)
		dispose()
	})
