...
describe('Effect', {
	...
	it('should run immediately', (): void => {
		...
		assert.deepStrictEqual(
			...
			'Effect should run immediately when created'
		)
	})
	...
	it('should run when dependencies change', (): void => {
		...
		assert.deepStrictEqual(
			...
			'Effect should run when its dependencies change'
		)
	})
	...
	it('should cleanup when disposed', (): void => {
		...
		assert.deepStrictEqual(
			...
			'Effect should not run after being disposed'
		)
	})
	...
	it('should handle dynamic dependencies', (): void => {
		...
		assert.deepStrictEqual(
			...
			'Initial execution should use a'
		)
		...
		assert.deepStrictEqual(
			...
			'Should react to a changes when condition is true'
		)
		...
		assert.deepStrictEqual(
			...
			'Should switch to b when condition is false'
		)
		...
		assert.deepStrictEqual(
			...
			'Should not react to a changes when condition is false'
		)
		...
		assert.deepStrictEqual(
			...
			'Should react to b changes when condition is false'
		)
	})
	...
	it('should only track direct dependencies, not intermediate values', (): void => {
		...
		assert.deepStrictEqual(
			...
			'Effect should update when b changes'
		)
		...
		assert.deepStrictEqual(
			...
			'Effect should update when a changes, even when not used in output'
		)
	})
	...
	it('should handle effects that read the same signal multiple times', (): void => {
		...
		assert.deepStrictEqual(
			...
			'Effect should handle multiple reads correctly'
		)
	})
	...
	it('should handle rapid consecutive updates efficiently', (): void => {
		...
		assert.deepStrictEqual(
			...
			'Effect should run after each update'
		)
	})
	...
	it('should execute effects in predictable order (registration order)', (): void => {
		...
		assert.deepStrictEqual(
			...
			'Effects should execute in registration order'
		)
	})
	...
	it('should not trigger effects when setting identical values', (): void => {
		...
		assert.deepStrictEqual(
			...
			'Effect should not re-run when value is unchanged'
		)
		...
		assert.deepStrictEqual(
			...
			'Effect should run when value changes'
		)
	})
})
