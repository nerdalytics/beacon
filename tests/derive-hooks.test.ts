...
import type { DeriveHooks } from '../src/hooks/index.ts'

describe('Derive Hooks', { concurrency: true, timeout: 1000 }, () => {
	...
	it('fires onCompute with previousValue', () => {
		...
		const $d = derive(() => $s.count * 10, {
			onCompute: (prev) => { prevValues.push(prev as number | undefined) },
		})
		assert.deepStrictEqual(prevValues, [undefined])
		$s.count = 2
		assert.deepStrictEqual(prevValues, [undefined, 10])
		$d.reactive = false
	})

	it('fires onCacheHit distinguishing fresh vs cached', () => {
		const hits: [unknown, boolean][] = []
		const $s = state({ count: 1 })
		const $d = derive(() => $s.count * 10, {
			onCacheHit: (value, cacheHit) => { hits.push([value, cacheHit]) },
		})
		const _v1 = $d.value
		const _v2 = $d.value
	...
	it('fires onDependencyChange when upstream state changes', () => {
		const changes: [object, PropertyKey][] = []
		const $s = state({ count: 0 })
		const $d = derive(() => $s.count * 2, {
			onDependencyChange: (target, prop) => { changes.push([target, prop]) },
		})
		$s.count = 5
		assert.ok(changes.length > 0)
		assert.strictEqual(changes.some((c) => c[1] === 'count'), true)
		$d.reactive = false
	})
	...
	it('isolates hook errors from derive functionality', () => {
		const $s = state({ count: 1 })
		const $d = derive(() => $s.count * 10, {
			onCompute: () => { throw new Error('hook error') },
		})
