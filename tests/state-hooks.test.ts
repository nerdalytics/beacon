...
import type { StateHooks } from '../src/hooks/index.ts'

describe('State Hooks', { concurrency: true, timeout: 1000 }, () => {
	...
	it('fires onRead with correct arguments', () => {
		...
		const $s = state({ name: 'Alice' }, {
			onRead: (prop, value, target) => { calls.push([prop, value, target]) },
		})
		const _v = $s.name
	...
	it('fires onWrite with correct arguments', () => {
		...
		const $s = state({ count: 0 }, {
			onWrite: (prop, oldValue, newValue, target) => {
				calls.push([prop, oldValue, newValue, target])
			},
		})
		$s.count = 5
		assert.strictEqual(calls.length, 1)
	...
	it('fires onDelete with correct arguments', () => {
		const calls: unknown[][] = []
		const $s = state({ temp: 'value' } as Record<string, unknown>, {
			onDelete: (prop, hadProperty, target) => {
				calls.push([prop, hadProperty, target])
			},
		})
		delete $s.temp
		assert.strictEqual(calls.length, 1)
	...
	it('fires onHas with correct arguments', () => {
		const calls: unknown[][] = []
		const $s = state({ name: 'Alice' }, {
			onHas: (prop, exists, target) => { calls.push([prop, exists, target]) },
		})
	...
	it('fires onOwnKeys with correct arguments', () => {
		...
		const $s = state({ a: 1, b: 2 }, {
			onOwnKeys: (keys, target) => { calls.push([keys, target]) },
		})
		Object.keys($s)
		assert.strictEqual(calls.length, 1)
	...
	it('propagates hooks to nested objects', () => {
		...
		const $s = state({
			...
			onWrite: (prop) => { writes.push(String(prop)) },
		})
		$s.user.name = 'Bob'
		$s.user.settings.theme = 'light'
		assert.deepStrictEqual(writes, ['name', 'theme'])
	})

	it('fires onRead when array mutating methods are accessed', () => {
		const reads: string[] = []
		const $s = state({ items: [1, 2, 3] }, {
			onRead: (prop) => { reads.push(String(prop)) },
		})
		$s.items.push(4)
