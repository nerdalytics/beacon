...
`read()` checks `currentSubscriber` and, if present, adds it to `subscribers`. `write()` compares using `this.equalityFn(this.value, newValue)`, updates the value, and calls `notifySubscribers()`. The signal itself is `Object.assign(read, { set: write, update })` — a function with methods.

### Custom equality in StateImpl

`StateImpl` stores `equalityFn` as an instance property, defaulting to `Object.is`. The `set()` method calls `this.equalityFn(this.value, newValue)` instead of the previously hardcoded `Object.is`. When `state()` or `protectedState()` receives an equality function, it is passed through to the `StateImpl` constructor.
