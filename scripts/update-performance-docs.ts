...
function parseTestOutput(output: string): Map<string, number> {
  ...
  const manyDepsRegex = /Handling \d+ dependencies with [\d,]+ iterations: ([\d.]+)ms\s+.*\s+Operations per second: ([\d,]+)\/s/;
...
function processTestResults(testOutputs: string[]): PerformanceMetric[] {
  ...
  metricInfo.set('Effect Triggers', { unit: 'ops/sec', category: 'Core', description: 'Effects running on state changes' });
  metricInfo.set('Batch Updates', { unit: 'ops/sec', category: 'Core', description: 'Updating multiple signals in batches' });
  metricInfo.set('Many Dependencies', { unit: 'ops/sec', category: 'Advanced', description: '100 dependencies, 100 iterations' });
  metricInfo.set('Batch Performance Ratio', { unit: 'x', category: 'Comparison', description: 'Speed improvement with batching vs. individual updates' });
  metricInfo.set('Batch Effect Reduction', { unit: 'x', category: 'Comparison', description: 'Reduction in effect runs with batching' });
...
function calculateTrends(history: PerformanceEntry[]): Map<string, { current: number, previous: number, change: number }> {
  if (history.length < 2) {
    return new Map();
  }
